...
using TcurrentPipe = sycl::ext::intel::pipe<TcurrentPipeID, float>; 
using TUpdatePipe = fpga_tools::PipeArray<TUpdatePipeID, float, 5, 2>; 
            ...
            if( StateIterPipe::template PipeAt<1>::read() == 1){  
              for(int i = 0; i < kSize; i++){   
                PRINTF("SMOOTH COST flag 0 \n"); 
                  delta = ARProducePipe::PipeAt<1>::read(); 
                PRINTF("SMOOTH COST flag 1 \n"); 
                MulRes = TUpdatePipe::PipeAt<1>::read(); 
        ...
        while(1){ 
          ...
          for(int i = 0; i < kSize; i++){
            fpga_tools::UnrolledLoop<2>([&i, & A_out](auto j){
                 TUpdatePipe::PipeAt<j>::write(A_out[i]);
  ...
  void operator()() const{
    while(1){
      ...
      if(state_iter == 1){
        ...
        for(int i = 0 ; i < kSize ; i++){
          T_current[i] += TUpdatePipe::PipeAt<0>::read(); 
...
fpga_tools::Autorun<ARTUpdateID> ar_kernelUpdate{selector, ARTUpdate{}}; 
fpga_tools::Autorun<ARMMULID> ar_kernel{selector, ARMMul{}};
fpga_tools::Autorun<ARsmoothCostID> ar_SmoothCost{selector, ARSmooth{}}; 
fpga_tools::Autorun<KsparseMulID> ar_SparseMul{selector, ARSparseMul{}}; 

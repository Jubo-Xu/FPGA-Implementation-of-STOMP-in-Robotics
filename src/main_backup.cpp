...
using pipearray_input = fpga_tools::PipeArray<PipeArray_Input_ID, float, N_main, DoF_main>;
...
// Define input kernel
fpga_tools::Autorun<ARInput_block_ID> ar_kernel_input(selector, Determine_and_Connections::AR_input_block<pipe_host_2_input, pipearray_input, pipearray_end_sig, pipearray_input_2_noisy, pipearray_input_2_update, pipearray_input_2_sparse, pipearray_input_2_smooth>{});
...
//
template <typename KernelID, typename Pipe, typename Pipe_loop>
sycl::event SubmitProducerKernel(sycl::queue& q, sycl::buffer<float, 1>& in_buf, sycl::buffer<float, 1>& loop_buf) {
  return q.submit([&](sycl::handler& h) {
    sycl::accessor in(in_buf, h, read_only);
    sycl::accessor loop(loop_buf, h, read_only);
    int size = in_buf.size();
    h.single_task<KernelID>([=] {
      for (int i = 0; i < 16; i++) {
        size_t index=0;
        fpga_tools::UnrolledLoop<3>([&index, &loop](auto idx_1) {
        Pipe_loop::template PipeAt<idx_1>::write(loop[2]); //i*3+index
        index++;
      });
...
try { 
    ...
    std::cout << "Running the Autorun kernel test\n";
    {
      ...
      sycl::buffer loop_buf(Loop);
      sycl::buffer out_buf(out_data);
      // submit data to kernel
      SubmitProducerKernel<ARProduceKernel_host_ID, pipe_host_2_input,pipearray_input>(q, in_buf, loop_buf);
    ...
    for(int i=0; i<DoF_main*N_main; i++){
      for (int j=0;j<3;j++){
        for (int k=0; k<16;k++){
      std::cout<<out_data[i*3*16+j*16+k]<<"\n";
        }
      }
    }
    std::cout<<"\n";

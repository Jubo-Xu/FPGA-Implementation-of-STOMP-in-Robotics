...
class PipeArray_ininit_ARTupdate_ID;
class PipeArray_inmmul_ARTupdate_ID;
...
using pipe_array_ininit_ARTupdate = fpga_tools::PipeArray<PipeArray_ininit_ARTupdate_ID, float, 5, 3>;
using pipe_array_inmmul_ARTupdate = fpga_tools::PipeArray<PipeArray_inmmul_ARTupdate_ID, float, 5, 3>;
...
using pipe_array_state_smooth = fpga_tools::PipeArray<PipeArray_state_smooth_ID, float, 1, 3>;
...
//fpga_tools::Autorun<ARTupdate_kernel_ID> ar_kernel_test_ARTupdate(selector, smooth::ARTUpdate<pipe_array_ininit_ARTupdate, pipe_array_inmmul_ARTupdate, pipe_array_out1_ARTupdate>{});

//Declare the kernel for SparseMMUL
// fpga_tools::Autorun<SparseMMUL_kernel_ID> ar_kernel_test_SparseMMUL(selector, smooth::ARSparseMul<pipe_array_in_SparseMMUL, pipe_array_out_SparseMMUL>{});
...
////////////////////////// TEST FOR ARTUpdate ////////////////////////////
// produce
template <typename KernelID, typename PipeInInit, typename PipeInMMul>
    ...
    h.single_task<KernelID>([=] {
      for(int i=0; i<2; i++){
        ...
        for(int j=0; j<5; j++){
            ...
            fpga_tools::UnrolledLoop<3>([&index_1, &IN_TRAJ, &j, &i](auto idx_1) {
              PipeInInit::template PipeAt<idx_1>::write(IN_TRAJ[i*3*5 + index_1*5 + j]);
              index_1++;
              //Pipe::template PipeAt<idx_1>::write(IN[index++][i]);
            });
...
*/
// consume
template <typename KernelID, typename Pipe_out1>
sycl::event SubmitConsume_ARTUpdate(sycl::queue& q, sycl::buffer<float, 1>& out_buf1){
  return q.submit([&](sycl::handler& h){
    sycl::accessor out1(out_buf1, h, write_only, no_init);
    //sycl::accessor out2(out_buf2, h, write_only, no_init);
    h.single_task<KernelID>([=] {
      for(int i=0; i<2; i++){
        for(int j=0; j<5; j++){
          size_t index_1 = 0;
          //size_t index_2 = 0;
          fpga_tools::UnrolledLoop<3>([&index_1, &out1, &i, &j](auto idx_2){
              // PipeInMMul::template PipeAt<idx_2>::write(IN_MUL[i*3*5 + index_2*5 + j]);
              // index_2++;
              out1[i*3*5+index_1*5+j] = Pipe_out1::template PipeAt<idx_2>::read();
              //out2[i*3*5+index_1*5+j] = Pipe_out2::template PipeAt<idx_2>::read();
...
// produce
template <typename KernelID, typename Pipe_in_State,typename Pipe_in_Mul, typename Pipe_in_sparse, typename Pipe_theta_initial, typename Pipe_delta_theta>
sycl::event SubmitProduce_Smooth(sycl::queue& q,sycl::buffer<float, 1>& buf_state,sycl::buffer<float, 1>& buf_mul, sycl::buffer<float, 1>& buf_sparse, sycl::buffer<float, 1>& buf_theta_initial, sycl::buffer<float, 1>& buf_delta_theta){
  return q.submit([&](sycl::handler& h){
    ...
    h.single_task<KernelID>([=] {
      for(int i=0; i<2; i++){
        ...
        if(i==0){
          Pipe_in_State::template PipeAt<1>::write(1);
        }
        else{
          Pipe_in_State::template PipeAt<1>::write(0);
        }
      }
    });
  });
...
std::vector<float> In_initial_ARTupdate = {
  1.0f, 2.0f, 3.0f, 4.0f, 5.0f,
  1.5f, 2.5f, 3.5f, 4.5f, 5.5f,
  2.0f, 3.0f, 4.0f, 5.0f, 6.0f,
...
std::vector<float> In_mull_ARTupdate = {
...
std::vector<float> In_state_smooth = {1, 1};
...
// std::cout << test <<std::endl;
 ...
 try {
    ...
    std::cout << "Running the Autorun kernel test\n";
    {
      ...
      /////////////////////////////////////////////////////////////////////
      ////////////////////// TEST FOR ARTUPDATE ///////////////////////////
      // sycl::buffer In_buf_traj_ARTupdate(In_initial_ARTupdate);
      // sycl::buffer In_buf_theta_ARTupdate(In_mull_ARTupdate);
      // sycl::buffer Out_buf_1_ARTupdate(Out_ARTupdate_1);
      // SubmitProduce_test_ARTUpdate<ARProduceKernel_ARTupdate_ID, pipe_array_ininit_ARTupdate, pipe_array_inmmul_ARTupdate>(q, In_buf_traj_ARTupdate, In_buf_theta_ARTupdate);
      // SubmitConsume_ARTUpdate<ARConsumeKernel_ARTupdate_ID, pipe_array_out1_ARTupdate>(q, Out_buf_1_ARTupdate);
      
      /////////////////////////////////////////////////////////////////////
      ////////////////////// TEST FOR SPARSEMMUL //////////////////////////
      // sycl::buffer In_buf_SparseMMUL(In_SparseMMUL);
      // sycl::buffer out_buf_SparseMMUL(Out_SparseMMUL);
      // SubmitProduce_SparseMMUL<ARProduceKernel_SparseMMUL_ID, pipe_array_in_SparseMMUL>(q, In_buf_SparseMMUL);
      ...
      sycl::buffer In_buf_state_smooth(In_state_smooth);
      sycl::buffer out_buf_smooth(out_smooth);
      SubmitProduce_Smooth<ARProduceKernel_smooth_ID,pipe_array_state_smooth,pipe_array_mul_smooth, pipe_array_sparse_smooth, pipe_array_theta_initial_smooth, pipe_array_delta_theta_smooth>(q, In_buf_state_smooth,In_buf_mul_smooth, In_buf_sparse_smooth, In_buf_theta_initial_smooth, In_buf_delta_theta_smooth);

...
class PipeArray_final1_ID;
class PipeArray_final2_ID;
//Define the Pipes
...
using pipe_rng_final_1 = fpga_tools::PipeArray<PipeArray_final1_ID, float, 1,12>;
using pipe_rng_final_2 = fpga_tools::PipeArray<PipeArray_final2_ID, float, 1,12>;
...
// fpga_tools::Autorun<AR_tri_rng_ID> ar_kernel_tri_rng(selector, RNG::AR_tri_rng_2_grng<pipe_delta_rng1,  pipearray_tri_rng_2_grng>{});
//fpga_tools::Autorun<AR_Gaussian_rng_kernel_ID> ar_kernel_gaussian_rng(selector, RNG::AR_Gaussian_RNG_single<pipe_alias_2_grng, pipearray_tri_rng_2_grng, pipe_grng_out>{});
...
fpga_tools::Autorun<AR_final_ID> ar_kernel_final(selector, RNG::AR_RNG_final<pipe_delta_rng1, pipe_rng_final_1,pipe_rng_final_2>{});
//////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////
...
template <typename KernelID, typename Pipe, typename Pipe2>
sycl::event SubmitConsume_RNG_final(sycl::queue& q, sycl::buffer<float, 1>& out_buf, sycl::buffer<float, 1>& out_buf2){
  return q.submit([&](sycl::handler& h){
    sycl::accessor out(out_buf, h, write_only, no_init);
    ...
    h.single_task<KernelID>([=] {
      for(int i=0; i<2; i++){
        size_t index = 0;
        fpga_tools::UnrolledLoop<12>([&index, &out, &out2, &i](auto idx){
            out[i*12+index] = Pipe::template PipeAt<idx>::read();
            out2[i*12+index] = Pipe2::template PipeAt<idx>::read();
            index++;
        });
      }
    });
...
try { 
    ...
    std::cout << "Running the Autorun kernel test\n";
    {
      ...
      sycl::buffer out_buf2(OUT2);
      SubmitConsume_RNG_final<RNG_consume_kernel_test_ID, pipe_rng_final_1, pipe_rng_final_2>(q, out_buf, out_buf2);
      // TEST FOR UNIFORM RNG 6 BIT 
